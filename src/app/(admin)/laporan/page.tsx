...
export default function LaporanKeuanganPage() {
  ...
  async function handleResubmit(id: string, e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const formData = new FormData();
      formData.append("REPORT_IMAGE", file);
      await reportsApi.resubmit(id, formData);
      load();
    } catch (err) {
      setError(
        err instanceof ApiError
          ? err.message
          : "Gagal mengunggah ulang bukti laporan",
      );
    } finally {
  ...
  async function handleFail(id: string) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Gagal memperbarui laporan",
      );
    } finally {
      setBusyId(null);
    }
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary">
            Laporan tutup buku bulanan kas RT.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Laporan bulanan akan muncul di sini setelah dibuat."
        />
      ) : (
          ...
          {reports.map((r) => (
            <Card key={r.id}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-y-2 text-sm">
                ...
                <span className="text-right font-medium text-secondary-dark">
              ...
              {canApprove && r.status === "open" && (
                <div className="mt-4 flex flex-col gap-3 border-t border-surface-tertiary pt-4">
                  {rejectingId === r.id ? (
                    <>
                      <Textarea
                        placeholder="Alasan laporan gagal ditutup"
                        value={reason}
